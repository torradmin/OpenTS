...
/*
 * Every sidebar gadget below is clicked on the screen but drawn onto the
 * sidebar surface, which is magnified onto the screen by the interface scale.
 */
...
/// <param name="x">Where the button sits, in sidebar surface
/// coordinates.</param> <param name="y">Where the button sits, in sidebar
/// surface coordinates.</param> <param name="nudge">How far below that its
/// shape is drawn.</param> <remarks>The position and size become the screen
/// ones, because that is what the button is clicked against; the draw offsets
/// carry its shape back into the surface's coordinates.
/// </remarks>
static void Place_Sidebar_Button(ShapeButtonClass &button, int x, int y,
                                 int nudge = 0) {
...
/// <remarks>The slot is never drawn from, only clicked on, so it carries the
/// screen rectangle alone.</remarks>
static void Place_Cameo_Slot(SidebarClass::StripClass::SelectClass &slot,
                             int column, int index) {
  using Strip = SidebarClass::StripClass;

  int const scale = UI_Scale();
  int const x = Map.Column[column].X;
  int const y = SidebarClass::SIDE_Y + Map.Column[column].Y +
                Strip::OBJECT_HEIGHT * index;
...
void SidebarClass::Reposition_Sidebar(void)
{
	 ...
	 * surface it is drawn on carries the interface at the size its artwork was made.
	 */
